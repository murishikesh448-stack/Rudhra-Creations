...
async function publishSubmission(payload) {
  if (!process.env.GITHUB_TOKEN) {
    throw createError(500, "GITHUB_TOKEN is not configured.");
  }
  ...
  if (!["Poem", "Story"].includes(type)) {
    throw createError(400, "Choose Poem or Story.");
  }

  if (!title || !author || !place || lines.length === 0) {
    throw createError(400, "Title, author, place, and content are required.");
  }
  ...
  const headers = {
    ...
    "User-Agent": "rudhra-creations-publisher"
  };

  const currentResponse = await fetch(url, { headers });
  if (!currentResponse.ok) {
    throw createError(500, "Could not read the public writing database.");
  }
  ...
  if (!updateResponse.ok) {
    throw createError(500, "Could not publish right now. Please try again.");
  }
